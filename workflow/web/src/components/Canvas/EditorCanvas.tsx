...
import { serializeToYaml } from "./dslSync";

export const nodeTypes = { canvasNode: CanvasNode };
const DEFAULT_NODES: Node[] = [
  { id: "llm_1", type: "canvasNode", position: { x: 80, y: 100 }, data: { nodeType: "llm", label: "AI Summary", description: "LLM Node - Intelligent Analysis", status: "running" } },
  { id: "tool_1", type: "canvasNode", position: { x: 320, y: 120 }, data: { nodeType: "tool", label: "Send Notification", description: "Tool Node - Webhook", status: "pending" } },
  { id: "cond_1", type: "canvasNode", position: { x: 80, y: 280 }, data: { nodeType: "condition", label: "Condition", description: "Condition Node", status: "pending" } },
];
...
export function EditorCanvas({ onDslChange }: EditorCanvasProps) {
  ...
  // Sync DSL on nodes/edges changes
  useEffect(() => {
    if (!onDslChange) return;
    if (!hasInited.current) { hasInited.current = true; }
    const yamlStr = serializeToYaml(nodes, edges);
    onDslChange(yamlStr);
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    if (hasCycle(nodes, updated)) { setCycleError("Cycle dependency detected"); setTimeout(() => setCycleError(null), 3000); return; }
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (ref.current) setSvgPaths(Array.from(ref.current.querySelectorAll<SVGPathElement>(".react-flow__edge-path")));
    }, 200);
  ...
  const onDrop = useCallback((e: React.DragEvent) => {
    ...
    setNodes(nds => [...nds, {
      id: `n_${Date.now()}`,
      type: "canvasNode",
      position: { x: e.clientX - rect.left - 88, y: e.clientY - rect.top - 20 },
      data: { nodeType: type, label: info.label, description: info.desc, status: "pending" },
    }]);
  }, []);
  ...
  useEffect(() => {
    const i = setInterval(() => {
      setNodes(nds => nds.map((n, idx) => ({
        ...
        data: {
          ...n.data,
          status: idx === 0 ? (Math.random() > 0.3 ? "success" : "running") : idx === 1 ? "success" : "failed",
        },
      })));
    }, 6000);
    ...
    <div ref={ref} style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {cycleError && (
        <div style={{
          ...
          zIndex: 50, background: "rgba(239,68,68,0.95)", color: "white",
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 2 }}>
        <ReactFlowProvider>
          <ReactFlow
            ...
            defaultEdgeOptions={{
              animated: true,
              style: { stroke: "rgba(129,140,248,0.4)", strokeWidth: 2 },
            }}
...
function Toggle({ side, onClick, label }: { side: "left" | "right"; onClick: () => void; label: string }) {
  const s: Record<string, number | string> = {
    position: "absolute", top: 12, zIndex: 10, width: 32, height: 32,
  ...
  return (
    <button
      ...
      onMouseLeave={e => { e.currentTarget.style.background = "var(--shell-surface)"; e.currentTarget.style.color = "var(--text-secondary)"; }}
    >
      {label}
    </button>
  );
...
function LeftPanel({ open }: { open: boolean }) {
  return (
    <div style={{
      position: "absolute", top: 0, left: 0, zIndex: 30, height: "100%",
      background: "var(--shell-surface)", borderRight: "1px solid var(--shell-border)",
      boxShadow: "4px 0 24px rgba(0,0,0,0.08)", width: 220, overflowY: "auto",
      transform: open ? "translateX(0)" : "translateX(-100%)",
      transition: "transform 250ms cubic-bezier(0.33,1,0.68,1)",
    }}>
      <div style={{ padding: 16, borderBottom: "1px solid var(--shell-border)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 13 }}>
        Node Types
      </div>
      <div style={{ padding: 8 }}>
        {Object.entries(NODE_META).map(([type, info]) => (
          <div
            key={type}
            draggable
            onDragStart={e => e.dataTransfer.setData("nodeType", type)}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, cursor: "grab", fontSize: 13, transition: "background 150ms" }}
            onMouseEnter={e => e.currentTarget.style.background = "#F1F5F9"}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}
          >
            <div style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>
              {info.icon}
            </div>
            <div>
...
function RightPanel({ open, selectedNode }: { open: boolean; selectedNode: Node | null }) {
  return (
    <div style={{
      position: "absolute", top: 0, right: 0, zIndex: 30, height: "100%",
      background: "var(--shell-surface)", borderLeft: "1px solid var(--shell-border)",
      boxShadow: "-4px 0 24px rgba(0,0,0,0.08)", width: 300, overflowY: "auto",
      transform: open ? "translateX(0)" : "translateX(100%)",
      transition: "transform 250ms cubic-bezier(0.33,1,0.68,1)",
    }}>
      <div style={{ padding: 16, borderBottom: "1px solid var(--shell-border)", display: "flex", justifyContent: "space-between", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 13 }}>
        ...
        <span style={{ color: "var(--text-muted)", fontWeight: 400, fontSize: 11 }}>
          {selectedNode ? String(selectedNode.data.label ?? "") : "Unselected"}
        </span>
      </div>
      {selectedNode ? (
        <ConfigPanel nodeType={String(selectedNode.data.nodeType ?? "")} />
      ) : (
